...
const NotExist=()=>{
  ...
  const handleSubmit = async(e) => {
    ...
    if (Object.keys(errors).length === 0) {
      ...
      try {
        const response = await axios.post('http://localhost:3000/JobRecruiterLogin', 
         {
          email,
          password,
         ...
         else
         {
          ...
          setUserExist(true);
          navigate(`/Recruiter/Profile/${response.data.id}`)
         }
      } catch (error) {
        console.error('Error sending search input:', error);
      }
      // Send formData to backend for further processing (e.g., saving to database)
  ...
  return (
    ...
    <div className="signUp">
      ...
      Don't Have an Account{/*<a href="/JobRecruiterSignUp">sign Up</a>*/}    <Link to="/JobRecruiterSignUp">Sign Up?</Link>
